...
Name *SVM Suite* stands for *Service Virtual Machine Suite*, and it represents a programming framework intended for human-computer interaction and automated reasoning. It is primarily intended for symbolic artificial intelligence programming, but it can also be used for other purposes.

*SVM Suite* includes a set of service virtual machines built on principles of service oriented programming ([SOP](https://en.wikipedia.org/wiki/Service-oriented_programming)) paradigm. SOP paradigm clearly distincts between different services dedicated to perform given tasks. Such services communicate between each other by passing messages. Benefits of this approach to programming is high modularity needed for code reuse, high agility in process of code development, and granular independence between services that can be invoked parallelly in a multitasking environment.
...
In this manner, the router directs messages from one service to another. Each `SOURCE` and `TARGET` section holds a service name, while each `EXP` section holds exchanged data.
...
*Compute-stateful.svm* is a service virtual machine providing computing data using state operations. Programs written in this service virtual machine resemble a sort of finite state machine. Programs are composed of a series of only one kind of statements which perform discrete compute steps in a process of computation. Each statement quotes the current step of the program execution, and points to the next step in execution, thus the order of statements is not relevant. Also, each step may input from or output to some memory cell, thus carrying on the computation. 

Finite automata, on whose layout Compute-stateful.svm is based, can perform various tasks like driving vending machines, elevators, traffic lights, combination locks, and many others. The limitations of finite state machines to perform only a subset of all possible tasks is surpassed by introducing reading from and writing to arbitrary memory cells during program execution. This places *Compute-stateful.svm* model of computation side by side with [Turing machines model](https://en.wikipedia.org/wiki/Turing_machine) which is known to be the most expressive model of computation.
